...
router.get('/', requireAuth, async (req: AuthRequest, res: Response) => {
  const keys = await query<{ id: number; name: string; created_at: string; key_preview: string }>(
...
router.delete('/:id', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  const key = await queryOne<ApiKey>('SELECT * FROM api_keys WHERE id = ? AND user_id = ?', [keyId, req.user!.id]);
